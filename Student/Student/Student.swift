...
class Student
{
    ...
    func validateObjectrating(_ object: String, _ rating: Double)throws -> Bool
    {
        if object.isEmpty { throw AddErrorStudent.emptyObject}
        if object.count < 3 || object.count > 10 { throw AddErrorStudent.TooBigSmolObject}
    ...
    init?(firstName: String, lastName: String, object: String, rating: Double)
    {
        do
        {
            let a = try validateFirstLastName(firstName, lastName)
            let b = try validateObjectrating(object, rating)
            if a && b
            {
                self.firstName = firstName
                self.lastName = lastName
